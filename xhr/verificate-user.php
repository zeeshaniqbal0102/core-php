...
//    );
    ...
    if (empty($_POST['name'])) {
        $error           = true;
        $data['message'] = $error_icon . "Enter your Name";
    }
    elseif(empty($_POST['text'])){
        $error           = true;
        $data['message'] = $error_icon . "Enter Message";
    }
     else {
        ...
        if (!file_exists($_FILES['passport']['tmp_name'])) {
            $error           = true;
            $data['message'] = $error_icon ."Please upload Passport/Id Card";
        }
        if (!file_exists($_FILES['photo']['tmp_name'])) {
            $error           = true;
            $data['message'] = $error_icon ."Please upload your photo";
        }
    ...
    if (!$error) {
        $registration_data = array(
            ...
            'type' => 'User',
            'seen' => 0,
            'created_at' => date("d") . '-' . date("m"). '-' . date('Y'),
        );
